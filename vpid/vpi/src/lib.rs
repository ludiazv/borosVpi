...
impl VpiTimes {
    ...
    // constructo VpiTimes from string <short> <space> <hold> <grace>
    pub fn from_string(args: String) -> VpiTimes {
        ...
        if p.len()>=1 && p[0]>20  && p[0]<=u16::max as i32  { r.short_tm=p[0] as u16 }
        if p.len()>=2 && p[1]>100 && p[1]<=u16::max as i32  { r.space_tm=p[1] as u16 }
        if p.len()>=3 && p[2]>0   && p[2]<=u8::max  as i32  { r.hold_tm= p[2] as u8  }
        if p.len()>=4 && p[3]>0   && p[3]<=u8::max  as i32  { r.grace_tm=p[3] as u8  }        
        r
    }
...
impl VpiLed {
    pub fn new(mode:u8, val:u8) -> VpiLed {
       VpiLed { led_mode: mode, led_val : val }
    }
}
...
impl VpiBuzz {
    fn new(f:u8,c:u8,b_t:u8,p_t:u8) -> VpiBuzz {
        VpiBuzz { buzz_freq:f , buzz_count: c, buzz_b_tm: b_t, buzz_p_tm: p_t }
    }
}
...
impl VpiStatus {
    pub fn has_changed(&self,&prev: &VpiStatus) -> bool {
        self.has_click          != prev.has_click           || self.has_error       != prev.has_error       ||
        self.has_irq            != prev.has_irq             || self.is_running      != prev.is_running      ||
        self.is_wdg_enabled     != prev.is_wdg_enabled      || self.is_wake_enabled != prev.is_wake_enabled ||
        self.is_wake_irq_enabled!= prev.is_wake_irq_enabled || self.out_value       != prev.out_value
...
struct VpiRegs {
  ...
  status        :u8, 	        ///< [RO] Status register    7 6 5 4 3 2 1 0
                                ...
                                ///<                         W: Watchdog enabled
                                ///<                         I: Interrupt pending
  flags         :u8,            ///< [RO] additional status flags 7 6 5 4 3 2 1 0
                                ///<                              1 0 X X X O W I
                                ///<                              I: Wake by IRQ enabled
                                ///<                              W: Wake enabled
                                ///<                              O: Ouput state
  crc           :u8,            ///< [RO] CRC8 of cofiguration registers
  buts          :[[u8;2];2],	///< [RO] Button clicks
  rpm           :u16,           ///< [RO] Fan estimated RPM
  err_count     :u8,           ///< [RO] Error count flags (for debuggin)
  uuid          :[u8;12],       ///< [RO] 96bit Unique ID.

  // RW section (Configuration and commands)
  pwm_freq      :u16,       ///< [RW] desired pwm frequency for fan and led 250-62500Hz
  rev_divisor   :u8,        ///< [RW] Number of pulses per revolution divisor.
  wdg           :u8,        ///< [RW] Time in seconds for highlevel watchdog 0=deactivated.
  wake          :u16,       ///< [RW] Autowake in minutes 0=no autowake
  times         :VpiTimes,  ///< [RW] times reg sections
  led           :VpiLed,    ///< [RW] led regs section
  buzz          :VpiBuzz,   ///< [RW] buzz regs section
...
pub struct Vpi {
    address     : u16,      /// Address    
    regs        : VpiRegs,  /// registers
    sregs       : VpiRegs,  /// shawdow registers (in endianness of the device)
...
impl Vpi {
    ...
    /// Retry function helper function. Retries the function until success w
    /// # Arguments
    ///   f: 
    fn retry<F>(mut f:F,debug:bool) -> Result<u32> 
       where F: FnMut() -> Result<()> {
        ...
        while retries>0 {
            res=f();
            if res.is_ok() { return Ok(I2C_RETRIES-retries) }
            if debug {
    ...
    fn read(&mut self,reg : u8, len: u8) -> Result<()>{
        if self.dev.is_none() { 
            ...
            let d=self.debug;
            if self.last_xfer().elapsed() < MIN_I2C_XFER_TIME  { thread::sleep(MIN_I2C_XFER_TIME); }
    ...
    fn atomic_write(&mut self,reg : u8, len:u8) -> Result<()> {
        ...
        if self.debug {
                ...
                let c=u.join(" ");
                println!("I2C-WR reg:0x{:02X} len:{} values:{}",reg,as_buff.len(),c);
        }
    ...
    pub fn get_uuid(&self) -> String {
        let u: Vec<String>=self.regs.uuid.iter().map( |b| format!("{:02X}",b)).collect();
        u.join("")
    }
    /// Sends all configuration registers to Vpi and send actulization command.
    pub fn config(&mut self) -> Result<()>  {
        ...
        self.wake_enable(self.regs.wake>0).cmd()?;
        Ok(())
    }
    ...
    pub fn check_status(&mut self,recover_flag :u8) -> Result<VpiStatus> {
        ...
        s.integrity = Vpi::status_integrity(self.regs.status) && Vpi::status_integrity(self.regs.flags);
        s.recover_type= recover_flag;
        if !s.integrity {
            Vpi::sleep_ms(10);
            self.read(offset_of!(VpiRegs,status) as u8,3)?;
            s.integrity= Vpi::status_integrity(self.regs.status) && Vpi::status_integrity(self.regs.flags);
            if !s.integrity { return Ok(s); }
        }
        s.crc=self.regs.crc;
        ...
        if s.has_rpm || s.has_click || s.has_error {
            ...
            let mut len =2u8;
            if s.has_click { len+=2; }
            if s.has_rpm && !s.has_click {
                first= offset_of!(VpiRegs,rpm) as u8;
            }
            if s.has_rpm && s.has_click { len+=3; }
            if s.has_error { len+=2; }
            Vpi::sleep_ms(5);
            self.read(first,len)?;
            if s.has_click {
        ...
        if s.crc != config_crc { // Configuration CRC mistmatch
            if self.debug {
                println!("CRC config mistmatch [Local:0x{:02X},Board:0x{:02X}] Sync launched",config_crc,s.crc);
            }
    ...
    pub fn recover(&mut self) -> Result<()> {
        ...
        Err(Vpi::err("Fatal error: Unable to recover connection with VPi"))
    ...
    pub fn monitor(&mut self) -> Result<VpiStatus> {
        ...
        match res {
                Err(LinuxI2CError::Io(_)) => {
                    ...
                    else { Err(Vpi::err("Fatal error: Unable to recover connection with VPi")) }
                },
                Ok(mut st) => {
                    st.recover_type=0;
                    if !st.is_running {
    ...
    pub fn run(&mut self,cmd:&VpiCmd,js:bool) -> Result<VpiCmdOutput> {
        match cmd {
            ...
            VpiCmd::Status => {
                ...
                if js { Ok(ret.to_json()) } else { Ok(ret) }
            },
            VpiCmd::Reset => {
                self.reset().cmd()?;
            ...
            VpiCmd::Wake(mins) => {
                self.wake(*mins).config()?;
                Ok(VpiCmdOutput::t_or_j(format!("Wake enabled for {} minutes after power off",mins).as_str(),js))
            },
            VpiCmd::IrqWake(en) => {
                self.wake_irq(*en).cmd()?;
                Ok(VpiCmdOutput::t_or_j(format!("Wake irq enabled={}",en).as_str(),js))
            },
            VpiCmd::Uuid => {
            ...
            VpiCmd::Wdg(secs) => {
                ...
                Ok(VpiCmdOutput::t_or_j(format!("Watchog set to {} seconds",secs).as_str(),js))
            },
            VpiCmd::Led(led) => {
                self.led_now(*led)?;
                Ok(VpiCmdOutput::t_or_j(format!("Led set to [mode={},value={}]",led.led_mode,led.led_val).as_str(),js))
            },
            VpiCmd::Fan(speed) => {
                self.fan_now(*speed)?;
                Ok(VpiCmdOutput::t_or_j(format!("Fan set to speed={}",speed).as_str(),js))
            },
            VpiCmd::Beep(buzz_pars) => {
                self.buzz_now(buzz_pars)?;
                Ok(VpiCmdOutput::t_or_j(format!("Issued {} beeps [mode:{}-{}-{}]",buzz_pars.buzz_count,buzz_pars.buzz_freq,buzz_pars.buzz_b_tm,buzz_pars.buzz_p_tm).as_str(),js))
            },
            VpiCmd::Timing(tim) => {
                self.timings(tim).config()?;
                unsafe {
                    Ok(VpiCmdOutput::t_or_j(format!("Button timming set to [short={}ms,space={}ms,hold={}s,grace={}s]",
                                                     tim.short_tm,tim.space_tm,tim.hold_tm,tim.grace_tm).as_str(),js))
                }
            }, 
            VpiCmd::Divisor(div) => { 
                self.rev_divisor(*div).config()?;
                Ok(VpiCmdOutput::t_or_j(format!("Fan RPM divisor set to {} per turn",div).as_str(),js))
            },
            VpiCmd::PwmFreq(pwmfreq) => {
                self.pwm_freq(*pwmfreq).config()?;
                Ok(VpiCmdOutput::t_or_j(format!("PWM frequency set to {} Hz",pwmfreq).as_str(),js))
            },
            VpiCmd::Output(val) => {
                self.output(*val).cmd()?;
                Ok(VpiCmdOutput::t_or_j(format!("Output value changed to {}",val).as_str(), js))
            }
        }
    }
