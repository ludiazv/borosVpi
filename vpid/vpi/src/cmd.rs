use crate::{VpiTimes,VpiBuzz,VpiStatus,VpiStats,VpiLed};
use serde_json::json;
use std::fmt;
        ...
        match v[0].to_lowercase().as_str() {
            ...
            "watchdog" => {
                if v.len()>=2 && v[1].parse::<i32>().is_ok() {
                    ...
                    if v>=0 && v<=u8::max as i32 { Some(VpiCmd::Wdg(v as u8)) } else { None }
                } else { None }
            },
            "wake"     => {
                if v.len()>=2 && v[1].parse::<i32>().is_ok() {
                    let v=v[1].parse::<i32>().unwrap() ;
                    if v>=0 && v<=u16::max as i32 { Some(VpiCmd::Wake(v as u16)) } else { None }
                } else { None }
            },
            "led"       => {
                ...
                if v.len()>=2 {
                   let m=match v[1].to_lowercase().as_str() {
                    ...
                    if v.len()>=3 {
                        ...
                        if tst>=0 && tst<=u8::max as i32 { val = tst as u8; }
                    }
                    if let Some(mo)= m { r = Some(VpiCmd::Led(VpiLed::new(mo,val))); }
                }
                r
            },
            "fan"       => {
                if v.len()>=2 && v[1].parse::<i32>().is_ok() {
                    ...
                    if  tst>=0 && tst<=u8::max as i32 { Some(VpiCmd::Fan(tst as u8)) } else { None }
                } else { None }
            },
            "beep"      => {
                let mut r:Option<VpiCmd> = None;
                if v.len() >= 2 {
                    ...
                    if v.len() >=3 { 
                        ...
                        if tst<=u8::max as i32 { count = tst as u8; }
                    }
                    if v.len() >=4 {
                        let tst=v[3].parse::<i32>().unwrap_or(b_ms as i32);
                        if  tst>=100 && tst<=2550 as i32 { b_ms= tst as u32; }
                    }
                    if v.len() >=5 {
                        let tst=v[3].parse::<i32>().unwrap_or(p_ms as i32);
                        if  tst>=100 && tst<=25500 as i32 { p_ms= tst as u32; }
                    }
                    if let Some(fe) = f { 
            ...
            "divisor" => {
                if v.len()>=2 && v[1].parse::<i32>().is_ok() {
                    let v=v[1].parse::<i32>().unwrap() ;
                    if v>=0 && v<=u8::max as i32 { Some(VpiCmd::Divisor(v as u8)) } else { None }
                } else { None }
            },
            "pwmfreq" => {
                if v.len()>=2 && v[1].parse::<i32>().is_ok() {
                    let v=v[1].parse::<i32>().unwrap() ;
                    if v>=2i32 && v<=62500i32 { Some(VpiCmd::PwmFreq(v as u16)) } else { None }
                } else { None }
            },
            "timing"  => {
                let p:Vec<i32>=v[1..].into_iter().map( |e| e.parse::<i32>().unwrap_or(-1)).collect();
                let mut t=VpiTimes::default();
                if p.len()>=1 && p[0]>20  && p[0]<=u16::max as i32  { t.short_tm=p[0] as u16 }
                if p.len()>=2 && p[1]>100 && p[1]<=u16::max as i32  { t.space_tm=p[1] as u16 }
                if p.len()>=3 && p[2]>0   && p[2]<=u8::max  as i32  { t.hold_tm = p[2] as u8  }
                if p.len()>=4 && p[3]>0   && p[3]<=u8::max  as i32  { t.grace_tm=p[3] as u8  }
